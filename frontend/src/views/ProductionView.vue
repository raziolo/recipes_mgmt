...
<template>
  <div>
    <div class="flex justify-between items-center">
      <div>
        <h2 class="text-3xl font-bold text-gray-800">Production</h2>
        <p class="mt-2 text-gray-600">Track and manage daily production tasks.</p>
      </div>
    ...
    <div class="mt-8 space-y-6">
      <div v-for="session in sessions" :key="session.id" class="bg-white rounded-xl shadow-sm border overflow-hidden">
        <div class="bg-gray-50 px-6 py-4 border-b flex justify-between items-center">
          <h3 class="font-bold text-lg text-gray-900">Session: {{ session.date }}</h3>
          <span :class="{
